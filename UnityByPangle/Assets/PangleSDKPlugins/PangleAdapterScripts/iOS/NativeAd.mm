...
- (void)buildupViewInterstial {
    ...
    NSString *imagePath = [bundle pathForResource:@"bu_close@2x" ofType:@"png"];
    [_closeButton setImage:[UIImage imageWithContentsOfFile:imagePath] forState:UIControlStateNormal];
    [_closeButton setBackgroundColor:[UIColor greenColor]];
...
- (void)refreshNativeAd:(BUNativeAd *)nativeAd {
    ...
    self.customview.frame = CGRectMake(self.customview.frame.origin.x, self.customview.frame.origin.y, self.customview.frame.size.width, self.customview.frame.size.width/2.0 + imageHeight);
...
void UnionPlatform_NativeAd_ShowNativeAd (void* nativeAdPtr,
    ...
    if ([NativeAd sharedInstance].adType == BUAdSlotAdTypeBanner && slotType == 0) {
        [nativeAdP buildupViewBanner];
        [nativeAdP refreshNativeAd:nativeAdP.nativeAd];
    } else if ([NativeAd sharedInstance].adType == BUAdSlotAdTypeInterstitial && slotType == 1) {
        [nativeAdP buildupViewInterstial];
        [nativeAdP refreshNativeAd:nativeAdP.nativeAd];
    } else if ([NativeAd sharedInstance].adType == BUAdSlotAdTypeFeed && slotType == 4) {
       [nativeAdP buildupViewFeed];
       [nativeAdP refreshNativeAd:nativeAdP.nativeAd];
   }
}
